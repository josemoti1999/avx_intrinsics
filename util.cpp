...
Convolution::Convolution(int m, int c, int r, int s, int sx, int sy, int px, int py)
{
  ...
  for(int i=0; i<M; i++)
    for(int j=0; j<C; j++)
      for(int k=0; k<R; k++)
        for(int l=0; l<S; l++)
          temp_weights[i][j][k][l] = (i*C*R*S+j*R*S+k*S+l)%256;
}
...
fmap* Convolution::conv2d_optimized(fmap* input_features, int size)
{
    //tiled OS conv2d_OS called inside this function after tiling
    ...
    *output_features = create_fmap(N, M, E, F);
    ...
    for(int e=0; e<E/size; e++){
        for(int f=0; f<F/size; f++){
              int e_min = e*size;
              int e_max = std::min((e+1)*size, E);
              int f_min = f*size;
              int f_max = std::min((f+1)*size, F);
              int E_tile = e_max-e_min;
              int F_tile = f_max-f_min;
              fmap* input_tile = (fmap*) malloc(sizeof(create_fmap(N, C,E_tile,F_tile)));
              *input_tile      = create_fmap(N, C, e_max-e_min, f_max-f_min);
              DATA (*temp_input_tile)[C][E_tile][F_tile] = (DATA (*)[C][E_tile][F_tile])input_features->data;
              for(int n=0; n<N; n++){
                  for(int c=0; c<C; c++){
                      for(int y=e_min; y<e_max; y++){
                          for(int x=f_min; x<f_max; x++){
                              temp_input_tile[n][c][y-e_min][x-f_min] = temp_inputs[n][c][y][x];
                          }
                      }
                  }
              }
              fmap* output_tiles = this->conv2d_OS(input_tile);  //for second layer both inputs, outputs same size
              DATA (*temp_output_tiles)[M][E_tile][F_tile] = (DATA (*)[M][E_tile][F_tile])output_tiles->data; 
              for(int n=0; n<N; n++){
                  for(int m=0; m<M; m++){
                      for(int y=0; y<E_tile; y++){
                          for(int x=0; x<F_tile; x++){
                              temp_outputs[n][m][e*size+y][f*size+x] = temp_output_tiles[n][m][y][x];
                          }
                      }
                  }
              }
...
fmap* Linear::linear(fmap* input_features)
{
    ...
    *output_features      = create_fmap(N, L, 1, 1);
    ...
    DATA (*temp_weights)[M] = (DATA (*)[M])weights;                   //changed to [l][m] to make avx easier
    ...
    for(int n=0; n<N; n++){
        for(int l=0; l<L; l++){
            ...
            for(int m=0; m<M/8; m++){
                ...
                mask=_mm256_set_epi32(index+8,index+7, index+6, index+5, 
                                      index+4,index+3, index+2,index+1);
                mm_inputs=_mm256_maskload_ps((float const*) &temp_inputs[n][m], mask);
                mm_weights=_mm256_maskload_ps((float const*) &temp_weights[l][m], mask);
                product=_mm256_mul_ps(mm_inputs, mm_weights);
                sum=_mm256_add_ps(sum, product);
            }
            _mm256_storeu_ps((float*) &array,sum);
            for(int qa=0;qa<8;qa++){
                temp_outputs[n][l]+=array[qa];
            }
        }
    }
...
fmap* Linear::linear_optimized(fmap* input_features) //not required
{
  return NULL;
}
...
fmap* AlexNet::forward_pass(fmap* input_features)
{
  ...
  temp = conv_layers[0]->conv2d_OS(temp);  //conv_2d conv2d_optimized conv2d_WS
  relu(temp);
  temp = maxpool_2d(temp, 3, 3, 2, 2);
  temp = conv_layers[1]->conv2d_optimized(temp,16);
  relu(temp);
  temp = maxpool_2d(temp, 3, 3, 2, 2);
